...
URONCLAD RULES OF DEBUGGING:
- ALWAYS START WITH THE FIRST ERROR.
- NEVER MOVE THE NEXT ERROR WITHOUT HAVING SOLVED THE PREVIOUS ONE.
- NEVER MOVE TO THE NEXT LEMMA WITHOUT VALIDATED FULL GREEN FOR THE CURRENT ONE. 
- NEVER EVER Use `sorry` or `admit` AT ANY POINT, ANYWHERE IN THE SCRIPT. 
- THESE WORDS ARE BANNED FROM THIS SCRIPT.
- GOLDEN RULE: APPROACH EVERY ERROR ONE BY ONE. FIX ONE ERROR AT A TIME, THEN ONE LEMMA AT A TIME.
- IT BEATIFULY ADDS UP.
